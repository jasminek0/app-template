...
3. **1단계: Epic 생성**: 
   ...
   - **중요**: 사용자가 명시적으로 브랜치를 만들지 말라고 요청하지 않는 한, 항상 Epic 0.0 "프로젝트 설정"을 첫 번째 Epic으로 포함하세요
...
7. **최종 출력 생성**: Epic, Task, 관련 파일, 참고 사항을 최종 Markdown 구조로 결합합니다.

8. **작업 목록 저장**: 생성된 문서를 `docs/TASKS-[기능명].md` 파일로 저장합니다.
...
- **Task ID 형식**: `TASK-[Epic번호]-[Task번호]` (예: TASK-01-01, TASK-02-03)
...
1. PRD를 주의 깊게 읽고 모든 요구사항을 이해하세요
...
6. 작업 목록을 `docs/TASKS-[기능명].md` 파일로 저장하세요
...
다음 단계:
- /execute-task TASK-00-01 명령어로 첫 번째 작업을 시작하세요
- 또는 /run-epic EPIC-00 명령어로 전체 Epic을 실행하세요
```
