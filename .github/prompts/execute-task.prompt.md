...
2. **TASKS 파일 확인**: 
   - `docs/tasks/TASKS-[기능명].md` 파일에서 해당 Task를 찾습니다
   - Task의 상세 내용과 요구사항을 확인합니다
   - Task가 속한 Epic의 컨텍스트를 이해합니다

3. **PRD 참조**: 
   - 필요한 경우 `docs/prd/PRD-[기능명].md`를 참조하여 기능 요구사항을 확인합니다
...
7. **Task 체크 표시**: 
   - `docs/tasks/TASKS-[기능명].md` 파일에서 해당 Task의 체크박스를 `- [x]`로 변경합니다
   - 변경사항을 저장합니다

8. **Epic 완료 확인 및 체크**:
   - 현재 Task가 속한 Epic의 모든 Task를 확인합니다
   - 모든 Task가 `- [x]`로 체크되어 있다면, Epic 헤더도 체크합니다
   - 예: `- [ ] ### EPIC-01: ...` → `- [x] ### EPIC-01: ...`
   - Epic이 완료되었음을 사용자에게 알립니다
